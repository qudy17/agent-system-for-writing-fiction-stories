...
ПРИОРИТЕТ 3 — ДОКУМЕНТЫ И ПРЕДМЕТЫ:
- Если письмо/документ уже цитировался в предыдущих сценах — в новой сцене
  не должно быть ДРУГОЙ цитаты из того же документа
  Пример нарушения: письмо в сцене 1 = "Граф, верните долг",
  в сцене 4 то же письмо = "если графиня не прекратит визиты" → ОТКЛОНИ
- Если предмет был в локации X в сцене N — в сцене N+1 он не может быть
  в локации Y без явного объяснения как он туда попал
  Пример нарушения: сапфир на столике в сцене 3, за зеркалом в сцене 5
  без описания перекладывания → ОТКЛОНИ

ПРИОРИТЕТ 4 — ПОВТОРЕНИЯ:
- Сцена не должна дословно или почти дословно воспроизводить описания
  из предыдущих сцен (запахи, улики, атмосферу)
- Если более 2 предложений почти совпадают с предыдущей сценой → ОТКЛОНИ:
  "Сцена повторяет детали из [название предыдущей сцены]"

ПРИОРИТЕТ 5 — ЛОГИКА:
- Персонаж не может быть в двух местах одновременно
- Мёртвый персонаж не может действовать
- Персонаж не может одновременно прятаться И продолжать спокойный осмотр
