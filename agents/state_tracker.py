...

{
  ...
  "sensory_details": {
    "smells": ["запах1 в локации1", "запах2 в локации2"],
    "sounds": ["звук1", "звук2"],
...
ВАЖНО для documents_mentioned:
- Если в тексте есть ПРЯМАЯ ЦИТАТА из письма/записки/документа — 
  точно воспроизведи её в exact_text
- Если один документ цитируется дважды с разным текстом — это два разных документа,
  дай им разные имена
"""
...
class StateTrackerAgent:
    ...
    def _fast_check(
        ...
        for doc_data in extracted.get("documents_mentioned", []):
            ...
            canon_normalized = _normalize_quote(canonical_text)[:60]
            scene_normalized = _normalize_quote(scene_text_fragment)[:60]

            # Проверяем в обе стороны: сцена содержит канон или наоборот
            is_same_document = (
                canon_normalized in scene_normalized
                or scene_normalized in canon_normalized
                or canon_normalized[:30] == scene_normalized[:30]
            )

            if not is_same_document and canon_normalized and scene_normalized:
                conflicts.append({
                    "type": "document_text",
                    "description": (
                        f"Текст документа '{doc_name}' изменён: "
                        f"в сцене приводится другая цитата"
                    ),
                    "canonical_fact": (
                        f"зафиксированный текст: "
                        f"'{canonical_text[:80]}'"
                    ),
                    "scene_fact": (
                        f"текст в сцене: "
                        f"'{scene_text_fragment[:80]}'"
                    ),
                    "severity": "critical",
                })
    ...
    def _update_bible(
        ...
        if isinstance(sensory, dict) and sensory:
            if "sensory_canon" not in self._bible:
                self._bible["sensory_canon"] = {}

            # Запахи по локациям — фиксируем первый упомянутый
            smells = sensory.get("smells", [])
            for smell_entry in smells:
                if not isinstance(smell_entry, str):
                    continue
                # Формат: "запах в локации" — берём как есть
                # Ключ — первые 20 символов как fingerprint
                smell_key = smell_entry[:20].lower().replace(" ", "_")
                if smell_key not in self._bible["sensory_canon"]:
                    self._bible["sensory_canon"][smell_key] = {
                        "description": smell_entry,
                        "established_in_scene": scene_index + 1,
                        "type": "smell",
                    }
                    logger.info(
                        "📚 Сенсорная деталь зафиксирована: '%s'",
                        smell_entry,
                    )
    ...
    def get_bible_for_writer(self) -> str:
        ...
        if sensory:
            lines.append(
                "СЕНСОРНЫЕ ДЕТАЛИ (зафиксированы — не противоречь им!):"
            )
            for key, detail in sensory.items():
                lines.append(
                    f"  • [сц.{detail.get('established_in_scene', '?')}] "
                    f"{detail.get('description', '')}"
                )
            lines.append(
                "  ⚠️  Запахи и атмосфера в уже описанных локациях "
                "должны совпадать с зафиксированными!"
            )
            lines.append("")
