...
class WriterAgent:
    ...
    # agents/writer.py — обновляем write_scene
    ...
    def write_scene(
        ...
        story_bible: str = "",
        approved_plan: str = "",        # ← одобренный план от ScenePlanner
        scene_transition: str = "",     # ← окончание предыдущей сцены
    ) -> Tuple[str, Dict[str, Any]]:

        previous_text = ""
        if approved_scenes:
            last_scene = approved_scenes[-1]
            previous_text = f"=== {last_scene['title']} ===\n{last_scene['text']}"

        memory_summary = _compact_memory(memory_context)

        user_prompt = (
            (f"{story_bible}\n\n" if story_bible else "")
            # Плавный переход — первым после Bible
            + (f"{scene_transition}\n\n" if scene_transition else "")
            # Одобренный план — обязательный скелет сцены
            + (
